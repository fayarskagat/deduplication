...
public class Variety {
    ...
    private String buildParams() {
        ...
        args.append("var collection = '" + inputCollection + "'");

        if(limit != null) {
            args.append(","+PARAM_LIMIT + " = " + limit);
        }

        if(maxDepth != null) {
            args.append(","+PARAM_MAXDEPTH + " = " + maxDepth);
        }

        if(query != null && !query.isEmpty()) {
            args.append(","+PARAM_QUERY + " = " + query);
        }

        if(sort != null && !sort.isEmpty()) {
            args.append(","+PARAM_SORT + " = " + sort);
        }

        if(outputFormat != null) {
            args.append(","+PARAM_OUTPUT_FORMAT + " = '" + outputFormat + "'");
        }

        if(persistResults) {
            args.append(","+PARAM_PERSIST_RESULTS + " = " + persistResults);
        }
    ...
    private String getVarietyPath() {
        ...
        return Paths.get("C:\\Program Files\\MongoDB\\Server\\3.0\\bin\\variety.js").toString();
    }
